...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert abs(1.0 - sigmoid(a) - sigmoid(-a)) < 1e-5
    assert sigmoid(0.0) == 0.5
    if abs(sigmoid(a) - 1.0) > 1e-5:
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    ls = [a, b, c]
    sorted_ls = sorted(ls)
    if lt(sorted_ls[0], sorted_ls[1]) and lt(sorted_ls[1], sorted_ls[2]):
        assert lt(sorted_ls[0], sorted_ls[2])
